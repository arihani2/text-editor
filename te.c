...
int determineEscapeCharacter(char c) {
    char seq[3];
...
int editorReadKey() {
    ...
    if (c == '\x1b') return determineEscapeCharacter(c);
...
int getCursorPosition(int* rows, int* cols) {
    ...
    while (i < sizeof(buffer) - 1) {
        if (read(STDIN_FILENO, &buffer[i], 1) != 1 || buffer[i] == 'R') break;
        i++;
    }
...
int editorRowCxToRx(erow* row, int cx) {
    ...
    int j = 0;
    for (j = 0; j < cx; j++) {
        if (row->chars[j] == '\t') rx += (TE_TAB_STOP - 1) - (rx % TE_TAB_STOP);
        rx++;
    }
...
int editorRowRxToCx(erow* row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t') cur_rx += (TE_TAB_STOP - 1) - (cur_rx % TE_TAB_STOP);
        cur_rx++;
...
void editorRowInsterChar(erow* row, int at, int c) {
...
void editorInsertChar(int c) {
    ...
    editorRowInsterChar(&E.row[E.cy], E.cx, c);
    E.cx++;
}
...
void addWelcomeMessage(struct abuf* ab) {
    char welcome[80];
    int welcomelen = snprintf(welcome, sizeof(welcome), "Text Editor editor -- version %s", TE_TAB_STOP);
